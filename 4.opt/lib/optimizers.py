...
class SGD(Optimizer):
    ...
    def __init__(
        self, params: List[Parameter], lr: float = 1.0, momentum: Optional[float] = None
    ):
...
class Adam(Optimizer):
    ...
    def __init__(
        ...
        self.betas = (
            betas  # betas are called rho (decay rates) in the DL book and lecture
        )
    ...
    def step(self) -> None:
        ...
        self.t += 1
        for p in self._params:
            # update exponential moving avg gradient and squared gradients
            p.state_dict["s"] = (
                self.betas[0] * p.state_dict["s"] + (1 - self.betas[0]) * p.grad
            )
            p.state_dict["r"] = (
                self.betas[1] * p.state_dict["r"] + (1 - self.betas[1]) * p.grad**2
            )
            # calculate bias corrected estimates, then delta_w
            s_hat = p.state_dict["s"] / (1 - self.betas[0] ** self.t)
            r_hat = p.state_dict["r"] / (1 - self.betas[1] ** self.t)
            delta_w = (-self.lr / (self.eps + np.sqrt(r_hat))) * s_hat
            # update
            p.data += delta_w
        # END TODO###################
...
def create_optimizer(
    name: str, params: List[Parameter], hyperparams: Dict[str, float]
) -> Optimizer:
