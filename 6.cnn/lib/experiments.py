...
def run_conv_experiment(
...
) -> Tuple[Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], Module]:
    ...
    # load training data
    x_train, x_val, x_test, y_train, y_val, y_test = load_mnist_data_subset(
        max_datapoints
    )
...
def run_shifted_conv_experiment(max_datapoints: int = 10000):
    ...
    # I have given my answers to the questions in submission.pdf
    ...
    x_val_mlp = flatten(x_val)
    x_train_mlp = flatten(x_train)
    x_val_shifted_mlp = flatten(x_val_shifted)
    # train the model
    train(
        mlp_model,
        loss_fn,
    ...
    try:
        ...
        raise RuntimeError(
            "Run the first experiment before you run the second experiment."
        )

    print(
        "Accuracy on data using an MLP: ",
        evaluate(x_val_mlp, y_val, mlp_model, loss_fn, batch_size)[0],
    )
    print(
        "Accuracy on shifted data using an MLP: ",
        evaluate(x_val_shifted_mlp, y_val, mlp_model, loss_fn, batch_size)[0],
    )
    print(
        "Accuracy on data using convolutional model: ",
        evaluate(x_val, y_val, conv_model, loss_fn, batch_size)[0],
    )
    print(
        "Accuracy on shifted data using convolutional model: ",
