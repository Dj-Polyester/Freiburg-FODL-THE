...
class CountingModel(nn.Module):
    ...
    def __init__(self, vocab_size, sequence_length, hidden):
        ...
        self.key_linear = nn.Linear(vocab_size, hidden)
        self.value_linear = nn.Linear(vocab_size, hidden)
        self.out_linear = nn.Linear(hidden, sequence_length + 1)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # attention_function.
        # START TODO #############
        # key and value has shapes (batch_size, sequence_length, hidden)
        key = self.key_linear(x)
        value = self.value_linear(x)
        # Repeat query batch_size times, reshaping to (batch_size, vocab_size, hidden), same shape as input
        # I preferred expand instead of repeat since expand does not copy tensor data
        # query_batch = self.query.repeat(x.shape[0], 1, 1)
        query_batch = self.query.expand(
            x.shape[0],
            self.query.shape[1],
            self.query.shape[2],
        )
        attention_outputs, attention_weights = attention_function(
            query_batch,
            key,
            value,
            None,
        )
        # raw_outputs has the shape (batch_size, vocab_size, hidden), same as input
        raw_outputs = attention_outputs + query_batch
        outputs_normed = self.norm(raw_outputs)
        outputs = self.out_linear(outputs_normed)
        # END TODO #############
