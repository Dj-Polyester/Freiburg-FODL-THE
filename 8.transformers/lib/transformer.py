...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.dec_pos_enc = nn.Parameter(
            torch.zeros((1, self.max_len + 1, self.embed_dim))
        )
    ...
    def forward(self, x1, x2):
        ...
        if not self.encoding_cached:
            ...
            # START TODO #############
            # enc_input_emb has shape (batch_size, self.max_len, self.embed_dim)
            enc_input_emb = self.enc_input_dense(x1)
            # END TODO #############

            # Add positional encoding to the embedded encoder inputs
            # START TODO #############
            # self.encoding has shape (batch_size, self.max_len, self.embed_dim)
            ...
            for i in range(self.enc_layers):
                ...
                # Self-attention
                # mha_output has same shape as self.encoding, (batch_size, self.max_len, self.embed_dim)
                # att_w has shape (num_heads, batch_size, self.max_len, self.max_len)
                mha_output, att_w = self.enc_att[i](
                    self.encoding,
                    self.encoding,
                    self.encoding,
                )
                # Add and normalize (we do not need this as the mha module does it)
                # add_normalize_out = self.enc_layer_norm[i](mha_output + self.encoding)
                # FFN block
                mha_output_doubled = self.enc_increase_hidden[i](mha_output)
                mha_output_doubled_relu = F.relu(mha_output_doubled)
                mha_output_ffn = self.enc_decrease_hidden[i](mha_output_doubled_relu)
                # Add and normalize
                enc_out = self.enc_layer_norm[i](mha_output_ffn + mha_output)
                # update
                self.encoding = enc_out
                # END TODO #############
        ...
        # START TODO #############
        # dec_input_emb = ...
        # dec_input_emb has shape (batch_size, self.max_len+1, self.embed_dim)
        dec_input_emb = self.dec_input_dense(x2)
        # END TODO #############

        # Add positional encodings to the embedded decoder inputs
        # START TODO #############
        # decoding has shape (batch_size, self.max_len+1, self.embed_dim)
        ...
        for i in range(self.dec_layers):
            ...
            # Self-attention
            # mha_output has same shape as decoding, (batch_size, self.max_len+1, self.embed_dim)
            # att_w has shape (num_heads, batch_size, self.max_len+1, self.max_len+1)
            mha_output, att_w = self.dec_att[i](
                decoding,
                decoding,
                decoding,
                mask=True,
            ...
            # Add and normalize (we do not need this as the mha module does it)
            # add_normalize_out = self.dec_layer_norm[i](mha_output + decoding)
            # Encoder-decoder attention,
            # query is the output of "Add and normalize" layer
            # values and keys are the output of the encoders
            # mha_output2 has same shape as decoding, (batch_size, self.max_len+1, self.embed_dim)
            # att_w2 has shape (num_heads, batch_size, self.max_len+1, self.max_len)
            mha_output2, att_w2 = self.enc_dec_att[i](
                mha_output,
                self.encoding,
                self.encoding,
            )
            attention = att_w2
            # Add and normalize (we do not need this as the mha module does it)
            # add_normalize_out2 = self.dec_layer_norm[i](mha_output2 + add_normalize_out)
            # FFN block
            mha_output2_doubled = self.dec_increase_hidden[i](mha_output2)
            mha_output2_doubled_relu = F.relu(mha_output2_doubled)
            mha_output2_ffn = self.dec_decrease_hidden[i](mha_output2_doubled_relu)
            # Add and normalize
            dec_out = self.dec_layer_norm[i](mha_output2_ffn + mha_output2)
            # Update
            decoding = dec_out
            # END TODO #############
        ...
        # START TODO #############
        # Linear
        # The output has the shape (batch_size, self.max_len+1, self.en_vocab_size)
        decoding = self.decoder_final_dense(decoding)
        # Softmax along dim 2, which is the vocabulary size (self.en_vocab_size)
        # (we do not need this as the loss function handles it)
