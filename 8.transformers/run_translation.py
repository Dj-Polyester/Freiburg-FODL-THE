...
class Task(object):
    ...
    def next_batch(self, batchsize=64, max_len=20, idx=None):
        ...
        for sample in en_minibatch_text:
            in_sample = self.embed(sample, self.en_dict, max_len=max_len, sos=True)
            out_sample = self.embed(sample, self.en_dict, max_len=max_len, eos=True)
            in_sample_embedded = np.eye(len(self.en_dict))[in_sample]
            out_sample_embedded = np.eye(len(self.en_dict))[out_sample]
            en_minibatch_in.append(in_sample_embedded)
            en_minibatch_out.append(out_sample_embedded)
