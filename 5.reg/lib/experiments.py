...
def build_model(
    linear_units: int, input_units: int = 784, num_classes: int = 10
) -> Module:
...
def build_model_dropout(
    linear_units: int,
    p_delete_input: float = 0.1,
    p_delete_hidden: float = 0.2,
    input_units: int = 784,
    num_classes: int = 10,
) -> Module:
    """Build a Sequential model with one hidden layer.
...
def train_models() -> (
    ...
    # START TODO ################
    # create loss, optimizer and train the model
    reg_cross_entropy = RegularizedCrossEntropy(L2Regularization(lambda_l2, params))
    ...
    # START TODO ################
    # create loss, optimizer and train the model
    reg_cross_entropy = RegularizedCrossEntropy(L1Regularization(lambda_l1, params))
