...
class Dropout(Module):
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # mask is 1 if active (not zeroed out), 0 (zeroed out) otherwise
        self.mask = 1 - np.random.binomial(1, self.p_delete, x.shape)

        return self.scale * self.mask * x
        # END TODO ################
    ...
    def backward(self, grad: np.ndarray) -> np.ndarray:
        ...
        # START TODO ################
        return self.scale * self.mask * grad
        # END TODO ################
...
class L1Regularization(Module):
    ...
    def forward(self) -> float:
        ...
        for param in self.params:
            sumParams += (np.abs(param.data)).sum()
        return self.lambd * sumParams
        # END TODO ################
...
class L2Regularization(Module):
    ...
    def forward(self) -> float:
        ...
        sumParams = 0
        for param in self.params:
            sumParams += 0.5 * self.lambd * (param.data**2).sum()
        return sumParams
        # END TODO ################
...
class RegularizedCrossEntropy(Module):
    ...
    def backward(self, _=None) -> np.ndarray:
        ...
        Returns:
            The gradient of this module.
        """
        self.reg_loss.backward(
            _
        )  # this updates parameter gradients, no grad w.r.t input
