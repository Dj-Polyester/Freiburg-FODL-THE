...
class Linear(Module):
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        assert (
            len(x.shape) == 2
        ), "x.shape should be (batch_size, input_size)" " but is {}.".format(x.shape)
        self.input_cache = x
    ...
    def backward(self, grad: np.ndarray) -> np.ndarray:
        ...
        x = self.input_cache
        assert (
            self.W.grad is not None and self.b.grad is not None
        ), "Gradients are None. Forgot to use zero_grad?"
        # START TODO ################
        self.W.grad += x.T @ grad
        self.b.grad += np.ones(grad.shape[0]) @ grad
