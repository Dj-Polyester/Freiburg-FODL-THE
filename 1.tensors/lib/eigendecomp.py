...
def get_matrix_from_eigdec(e: np.ndarray, V: np.ndarray) -> np.ndarray:
    ...
    """
    # START TODO #################
    # obtain diagonal eigenvalue matrix
    ediag = np.eye(len(e)) * e
    # because V is orthonormal, inverse is transpose
    Vinv = V.T
    # as long as A is diagonalizable, it is equal to the expression below
    return V @ ediag @ Vinv
    # END TODO ###################
...
def get_inverse(e: np.ndarray, V: np.ndarray) -> np.ndarray:
    ...
    # obtain inverse of diagonal eigenvalue matrix, which is 1/e
    ediaginv = np.eye(len(e)) * (1 / e)
    # because V is orthonormal, inverse is transpose
    Vinv = V.T
    # as long as A is diagonalizable, its inverse is equal to the expression below
    return V @ ediaginv @ Vinv
    # END TODO ###################
