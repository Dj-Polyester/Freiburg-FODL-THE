...
def get_mu_logvar(
    model: VAE, test_loader: data.DataLoader, device: str = "cpu"
...
def sample_on_grid(
    ...
    step_size = np.array(
        [
            (latent0_max - latent0_min) / grid_size,
            (latent1_max - latent1_min) / grid_size,
        ]
    )
    ...
    # meshgrid shape is (step_size[0],step_size[1],2) or (grid_size,grid_size,2)
    decoded = []
    meshgridReduced = mesh_grid.reshape(grid_size**2, mesh_grid.shape[2])
    decoded = model.decode(meshgridReduced)
...
def plot_latent_space(
    ...
    grid_size = 20
    decoded, step_size = sample_on_grid(
        model,
        latent_min[0],
        latent_max[0],
        latent_min[1],
    ...
    plt.errorbar(
        mean_mu_scaled[0],
        mean_mu_scaled[1],
        yerr=mean_stddev_scaled[1],
        xerr=mean_stddev_scaled[0],
        linestyle="None",
    )
    # plot the means
    ...
    for i, txt in enumerate([str(i) for i in range(0, 10)]):
        plt.annotate(
            txt,
            (mean_mu_scaled[0][i] + 3, mean_mu_scaled[1][i] - 3),
            color="blue",
