...
class CrossEntropyLoss(Module):
    ...
    def forward(self, preds: np.ndarray, labels: np.ndarray) -> float:
        ...
        # START TODO #################
        # get reduction axes
        reduction_axes_mean = 0
        reduction_axes_sum = tuple(range(1, len(labels.shape)))
        # compute the sum of losses along each batch
        unreduced_losses = (-labels * np.log(preds)).sum(axis=reduction_axes_sum)
        # average over batches
        return unreduced_losses.mean(axis=reduction_axes_mean)
        # END TODO ##################
