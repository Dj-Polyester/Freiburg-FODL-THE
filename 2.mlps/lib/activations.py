...
class Softmax(Module):
    ...
    def _softmax(self, z: np.ndarray) -> np.ndarray:
        ...
        # calculate max for each batch and align them to batches
        zMax = z.max(axis=reduction_axes)[None, :].T
        # normalize for numerical stability
        z -= zMax

        ez = np.e**z

        # calculate sum for each batch and align them to batches
        ezSum = ez.sum(axis=reduction_axes)[None, :].T
        # normalize
        h = ez / ezSum
        # END TODO###################
        return h
